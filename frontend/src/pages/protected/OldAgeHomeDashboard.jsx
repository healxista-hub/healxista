...
import { 
    Home, Users, Heart, Calendar, MessageCircle, 
    BarChart3, TrendingUp, ShieldCheck, HeartPulse, 
    Soup, Coffee, Thermometer, ArrowRight, UserPlus, Activity,
...
const OldAgeHomeDashboard = () => {
    ...
    // Feed for new admissions
    const [admissionsFeed, setAdmissionsFeed] = useState([]);
    useEffect(() => {
        fetchApi(`/api/bookings/assigned`).then(data => {
            if (data) setAdmissionsFeed(Array.isArray(data) ? data.filter(i => i.status === 'Pending').slice(0, 5) : []);
        });
    }, [user]);
        ...
        <div className="min-h-screen bg-slate-50/50 p-4 md:p-8 space-y-8 pb-12">
                ...
                <div className="relative p-6 md:p-14 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
                    <div className="text-white space-y-6 max-w-2xl">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-black uppercase tracking-[0.2em]">
                            <ShieldCheck className="h-4 w-4 text-emerald-400" /> Platinum Care Facility
                        </div>
                        <h1 className="text-3xl md:text-6xl font-black tracking-tight leading-none text-white">
                            {user?.name || 'Serenity'} <span className="text-red-500">Living</span>
                        </h1>
                        <p className="text-red-100/80 text-lg md:text-2xl max-w-xl leading-relaxed font-medium">
                            Providing a compassionate environment for the elderly. Monitor resident wellness and manage new admission inquiries.
                        </p>
                        <div className="flex flex-wrap gap-4 pt-2">
                            <div className="flex flex-col">
                                <span className="text-red-400/60 text-[10px] font-black uppercase tracking-[0.15em]">Facility License</span>
                                <span className="text-white font-bold">{user?.customId || 'CARE-9902'}</span>
                            </div>
                            <div className="h-8 w-px bg-white/20 self-end mb-1" />
                            <div className="flex flex-col">
                                <span className="text-red-400/60 text-[10px] font-black uppercase tracking-[0.15em]">Admin Status</span>
                                <span className="text-white font-bold flex items-center gap-2">
                                     <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse"></span> System Active
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="bg-white border-slate-100 shadow-sm hover:shadow-xl transition-all group rounded-3xl overflow-hidden p-5 md:p-6 border-l-4 border-l-red-500">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Total Applications</p>
                </Card>
                ...
                <Card className="bg-white border-slate-100 shadow-sm hover:shadow-xl transition-all group rounded-3xl overflow-hidden p-6 border-l-4 border-l-emerald-500">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Confirmed Residents</p>
                </Card>
                ...
                <Card className="bg-white border-slate-100 shadow-sm hover:shadow-xl transition-all group rounded-3xl overflow-hidden p-6 border-l-4 border-l-amber-500">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Pending Inquiries</p>
                </Card>
                ...
                <Card className="bg-slate-900 border-slate-800 shadow-lg group rounded-3xl overflow-hidden p-6 text-white text-center">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Cancelled Files</p>
                </Card>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4 pb-6 border-b border-slate-50">
                        <div>
                            <h3 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                                <BarChart3 className="h-6 w-6 text-red-600" /> Admission Metrics
                            </h3>
                            <p className="text-sm text-slate-400 font-bold tracking-widest uppercase mt-1">7-Day Application Activity</p>
                        </div>
                    </div>
                    <div className="flex-1 h-64 md:h-80 w-full">
                         {stats.byDayTrend && stats.byDayTrend.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-slate-300 gap-3 border-2 border-dashed border-slate-100 rounded-3xl">
                                <Home className="h-12 w-12 opacity-10" />
                                <p className="text-sm font-bold uppercase tracking-widest">Awaiting admission data...</p>
                            </div>
                         )}
                    </div>
                </Card>
                    ...
                    <Card className="bg-gradient-to-br from-red-600 to-rose-700 rounded-[2.5rem] shadow-2xl p-8 text-white relative overflow-hidden">
                         ...
                         <div className="relative z-10 space-y-6">
                            <h3 className="text-2xl font-black flex items-center gap-3">
                                <ShieldCheck className="h-7 w-7 text-white" /> Compassionate Care
                            </h3>
                            <p className="text-red-50/80 text-lg leading-relaxed font-medium">
                               Our facility is dedicated to the dignity and comfort of every resident. All care protocols are strictly adhered to.
                            </p>
                            <div className="pt-4 border-t border-white/10 flex items-center gap-4">
                                <div className="text-center">
                                    <p className="text-2xl font-black">{stats.accepted}</p>
                                    <p className="text-[10px] font-bold text-red-100/60 uppercase tracking-widest">Residents</p>
                                </div>
                                <div className="h-10 w-px bg-white/10" />
                                <div className="text-center">
                                    <p className="text-2xl font-black">{stats.total}</p>
                                    <p className="text-[10px] font-bold text-red-100/60 uppercase tracking-widest">Applications</p>
                                </div>
                            </div>
                         </div>
                    </Card>

                    {/* Pending Inquiry Feed */}
                    <Card className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col min-h-[400px]">
                        <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                            <h3 className="font-black text-slate-800 flex items-center gap-2">
                                <UserPlus className="h-5 w-5 text-orange-500" /> Admissions
                            </h3>
                            <span className="text-[10px] font-black bg-orange-50 text-orange-600 px-3 py-1 rounded-full">{admissionsFeed.length} PENDING</span>
                        </div>
                        <div className="p-4 space-y-3 flex-1 overflow-y-auto max-h-[300px]">
                            {loading ? (
                                ...
                                <div className="flex flex-col items-center justify-center p-8 text-center text-slate-400 gap-2">
                                    ...
                                    <p className="text-xs font-bold">No pending admissions.</p>
                                </div>
                            ) : (
                                admissionsFeed.map(inquiry => (
                                    <div key={inquiry.booking_id} className="p-4 rounded-3xl bg-slate-50 border border-slate-100 flex items-center justify-between group hover:bg-white hover:shadow-xl hover:scale-[1.02] transition-all">
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 rounded-2xl bg-white border border-slate-100 flex items-center justify-center font-black text-orange-600 overflow-hidden shrink-0">
                                                {inquiry.user_profile_image ? (
                                                    <img src={`/uploads/${inquiry.user_profile_image}`} alt="" className="w-full h-full object-cover" />
                                                ) : (
                                                    inquiry.userName?.charAt(0)
                                                )}
                                            </div>
                                            <div>
                                                <p className="text-xs font-black text-slate-800">{inquiry.userName}</p>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase">Inquiry ID #{inquiry.booking_id?.toString().slice(0,6)}</p>
                                            </div>
                                        </div>
                                        <Button size="icon" variant="ghost" className="h-8 w-8 rounded-xl bg-white text-orange-600 hover:bg-orange-50 shadow-sm" onClick={() => setActiveChatBooking(inquiry)}>
...
export default OldAgeHomeDashboard;
