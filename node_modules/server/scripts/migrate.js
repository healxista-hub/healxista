...
export async function runMigrations() {
    try {
        ...
        // 2.7 Repair specialized provider tables
        console.log('🚀 Verifying specialized provider columns...');
        await db.query(`
            DO $$ 
            BEGIN 
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='old_age_homes' AND column_name='admin_name') THEN
                    ALTER TABLE old_age_homes ADD COLUMN admin_name VARCHAR(255);
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='lab_tests' AND column_name='owner_name') THEN
                    ALTER TABLE lab_tests ADD COLUMN owner_name VARCHAR(255);
                END IF;
            END $$;
        `);
        console.log('✅ specialized provider columns verified');
        ...
        await db.query(`
            CREATE TABLE IF NOT EXISTS home_cares (
